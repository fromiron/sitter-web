...
export function TextInput({
    ...
    return (
        <div className={`${isFit ? 'w-fit' : 'w-full'}  max-w-xs form-control group`}>
...
export function TextAreaInput({label, placeholder, Icon, register, errorMsg, rows}: TextAreaInterface) {
    return (
        <div className="w-full max-w-xs form-control group h-auto">
            <label className="label">
        <span className="flex items-center align-bottom label-text group-focus-within:text-primary">
        <Icon className="mr-2 transition duration-500 text-base-100 group-focus-within:text-primary"/>
            {label}
            ...
            <textarea
                placeholder={placeholder}
                rows={rows}
                className="w-full max-w-xs h-auto py-2 transition border-2 input group-focus-within:border-primary resize-none"
                {...register}
            />
            {errorMsg ? (
                <div className="ml-2 text-sm text-error">{errorMsg.toString()}</div>
...
export function SelectInput({label, children, Icon, register}: SelectInputInterface) {
    return (
